...
/**
 * Base class that represents a row from the 'student_tb' table.
 *
 * 
 *
...
abstract class Student implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\bdxe\\Map\\StudentTableMap';
    ...
    public function setPassword($v)
    {
        ...
        if ($this->password !== $v) {
            $this->password = $v;
            $this->modifiedColumns[StudentTableMap::COL_PASSWORD] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : StudentTableMap::translateFieldName('Email', TableMap::TYPE_PHPNAME, $indexType)];
            $this->email = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : StudentTableMap::translateFieldName('Password', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 4; // 4 = StudentTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\bdxe\\Student'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {                
                $dataFetcher = $con->query('SELECT student_tb_SEQ.nextval FROM dual');
        ...
        if ($this->isColumnModified(StudentTableMap::COL_PASSWORD)) {
            $modifiedColumns[':p' . $index++]  = 'password';
        }

        $sql = sprintf(
            'INSERT INTO student_tb (%s) VALUES (%s)',
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\bdxe\Student The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(StudentTableMap::COL_PASSWORD)) {
            $criteria->add(StudentTableMap::COL_PASSWORD, $this->password);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \bdxe\Student (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \bdxe\Student Clone of current object.
     * @throws PropelException
     */
    ...
    public function setSubscriptions(Collection $subscriptions, ConnectionInterface $con = null)
    {
        ...
        foreach ($subscriptionsToDelete as $subscriptionRemoved) {
            $subscriptionRemoved->setStudent(null);
        }
     ...
     * @return ObjectCollection|ChildSubscription[] List of ChildSubscription objects
     */
    public function getSubscriptionsJoinCourse(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildSubscriptionQuery::create(null, $criteria);
